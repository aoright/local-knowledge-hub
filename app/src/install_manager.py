...
- Before the final response, proactively review the current user's own messages; do not wait for the user to say “remember this.” If and only if the user explicitly authored a durable decision, fact, constraint, or runbook, call `knowledge_capture` automatically with `scope=auto` and preserve the user's statement as evidence. A task request, assistant implementation result, or fact already represented by project files is not a memory. Statements explicitly applying to all projects or globally may enter a global scope; otherwise they remain in the current project. Never capture ordinary chat, guesses, transient debugging, secrets, or web claims.
